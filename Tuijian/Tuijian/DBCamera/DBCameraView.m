...
@property (nonatomic, strong) UIView *bottomTransparentView;
...
- (void) defaultInterface
{
    ...
    [self addSubview:self.bottomTransparentView];
...
- (UIView *) topTransparentView
{
    if ( !_topTransparentView ) {
        _topTransparentView = [[UIView alloc] initWithFrame:(CGRect){ 0, CGRectGetMinY(IS_RETINA_4 ? previewFrameRetina_4 : previewFrameRetina), CGRectGetWidth(self.bounds), (IS_RETINA_4 ? topTransparentHeightRetina_4 : topTransparentHeightRetina) }];
        _topTransparentView.backgroundColor = [UIColor blackColor];
        if (IS_RETINA_4) {
            _topTransparentView.alpha = 0.8;
        }
    }
    return _topTransparentView;
}

- (UIView *) bottomTransparentView
{
    if ( !_bottomTransparentView ) {
        CGFloat bottomBarY = CGRectGetMaxY( IS_RETINA_4 ? previewFrameRetina_4 : previewFrameRetina );
        CGFloat newY = IS_RETINA_4 ? (bottomBarY - bottomTransparentHeightRetina_4) : (bottomBarY - bottomTransparentHeightRetina);
        _bottomTransparentView = [[UIView alloc] initWithFrame:(CGRect){ 0, newY, CGRectGetWidth(self.bounds), (IS_RETINA_4 ? bottomTransparentHeightRetina_4 : bottomTransparentHeightRetina)}];
        _bottomTransparentView.backgroundColor = [UIColor blackColor];
        if (IS_RETINA_4) {
            _bottomTransparentView.alpha = 0.8;
        }
    }
    return _bottomTransparentView;
}
